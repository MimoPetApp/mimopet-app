...
export default {
  ...
  computed: {
    ...mapState('training', ['question', 'loadingTrainings']),
    feedbackIcon () {
      return petIcon
    },
    checkedIcon () {
  ...
  methods: {
    ...mapActions('training', ['ActionGetQuestion']),
    ...mapActions('question', ['ActionUpdateQuestionStepCompleted']),
    ...
    */
    ...
    async stepDone () {
      if (!this.isStepDone()) {
        // Atualizar o status
        const res = await this.ActionUpdateQuestionStepCompleted({
          id: this.questionID,
          body: {
            completed: true
          }
